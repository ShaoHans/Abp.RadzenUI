...
如果你只需要消息实体和 EF Core 映射能力，也可以单独安装以下包：
```shell
dotnet add package AbpRadzen.Messages.EntityFrameworkCore
```

#### 2. 注册 DbContext
完整 UI 包已经在 `AbpRadzenUIModule` 中注册了 `MessageDbContext`。如果你仅在自己的项目中集成 EF Core 包，则需要在业务 DbContext 中补充 `DbSet`，并调用 `ConfigureMessages()`。
